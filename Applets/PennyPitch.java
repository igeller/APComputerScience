...
public class PennyPitch extends JApplet implements ActionListener
{
	...
	JTextArea output = new JTextArea(9, 20);  // 10 rows & 20 cols (more cols may be needed for PCs)

	// NO CODE TO WRITE IN THIS METHOD
	public void init()
	{
		
		resize(300, 300);  // PC laptops may need to tweak figures
		...
		for (int row = 0; row < board.length; row++)
		{
			for(int col = 0; col < board[row].length; col++)
			{
				if (row == 0 || row == board.length-1)// && (col == 0 || col == board[row].length-1))
					board[row][col] =  new Square("1", false);
				else if (col == 0 || col == 4)
					board[row][col] =  new Square("1", false);
				else if ((row == 1 || row == 3) && (col == 1 || col == 2 || col ==3))
					board[row][col] =  new Square("2", false);
				else if (row ==2 && (col == 1 || col == 3))
					board[row][col] =  new Square("2", false);
				else if (row == 2 && col == 2)
					board[row][col] =  new Square("3", false);
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if (turnsLeft == 0)
		{
			JOptionPane.showMessageDialog(PennyPitch.this, 
					"Game Over!\n Score: " + totalPoints,
